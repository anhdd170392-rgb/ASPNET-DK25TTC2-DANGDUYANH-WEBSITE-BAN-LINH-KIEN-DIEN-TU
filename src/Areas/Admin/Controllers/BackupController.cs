...
namespace LTW.Areas.Admin.Controllers
{
    ...
    public class BackupController : Controller
    {
        ...
        public ActionResult Index()
        {
            try
            {
                // Lấy danh sách lịch sử backup từ database
                using (SqlConnection connection = new SqlConnection(connectionString))
                {
                    connection.Open();
                    string query = "SELECT BackupID, BackupName, BackupPath, Description, CreatedAt FROM Backup_History ORDER BY CreatedAt DESC";
                    using (SqlCommand cmd = new SqlCommand(query, connection))
                    using (SqlDataReader reader = cmd.ExecuteReader())
                    {
                        var history = new List<Backup_History>();
                        while (reader.Read())
                        {
                            history.Add(new Backup_History
                            {
                                BackupID = reader.GetInt32(0),
                                BackupName = reader.GetString(1),
                                BackupPath = reader.GetString(2),
                                Description = reader.IsDBNull(3) ? "" : reader.GetString(3),
                                BackupDate = reader.IsDBNull(4) ? (DateTime?)null : reader.GetDateTime(4)
                            });
                        }
                        return View(history);
                    }
            ...
            return View(new List<Backup_History>());
        }
