...
namespace LTW.Areas.Admin.Controllers
{
    ...
    public class SanPhamsController : Controller
    {
        ...
        public ActionResult Delete(int id, FormCollection collection)
        {
            ...
            var D_sp = data.SanPhams.FirstOrDefault(m => m.MaSP == id);

            if (D_sp != null)
            {
                // Tìm và xóa các bản ghi khuyến mãi liên quan trước
                var kmList = data.SanPham_KhuyenMais.Where(km => km.MaSP == id).ToList();
                data.SanPham_KhuyenMais.DeleteAllOnSubmit(kmList);
